...
import { get_comment_by_id as get_marker_comment_by_id } from '../repos/marker-comments.repo';

export async function MarkerCommentExists(
  request: Request,
  response: Response,
  next: NextFunction
) {
  const comment_id = parseInt(request.params.comment_id, 10);
  const comment_model = await get_marker_comment_by_id(comment_id);
  if (!comment_model) {
