...
import * as RepliesRepo from '../repos/marker-comment-replies.repo';
import {
  user_attrs_slim
} from '../../_common/common.chamber';
import {
  MarkerCommentReplies,
  MarkerCommentReplyReactions
} from '../models/marker.model';
import { Users } from '../../_common/models/user.model';
import { COMMON_REACTION_TYPES } from '../../_common/enums/common.enum';

export class MarkerCommentRepliesService {
  /** Request Handlers */
  ...
  static async get_comment_replies_count(request: Request, response: Response) {
    ...
    const replies_count = await MarkerCommentReplies.count({ where: { comment_id } });
    return response.status(HttpStatusCode.OK).json({
      data: replies_count
    });
  }
  ...
  static async get_user_reaction(request: Request, response: Response) {
    ...
    const reply_reaction = await MarkerCommentReplyReactions.findOne({
      where: {
        reply_id,
        owner_id: user_id
      }
  ...
  static async toggle_user_reaction(request: Request, response: Response) {
    ...
    let reply_reaction = await MarkerCommentReplyReactions.findOne({
      where: {
        reply_id,
        owner_id: you.id
      }
    ...
    if (!reply_reaction) {
      ...
      reply_reaction = await MarkerCommentReplyReactions.create(<any> {
        reaction,
        reply_id,
        owner_id: you.id
      });
  ...
  static async get_reply_reactions_counts(request: Request, response: Response) {
    ...
    const like_count = await MarkerCommentReplyReactions.count({ where: { reply_id, reaction: COMMON_REACTION_TYPES.LIKE } });
    const love_count = await MarkerCommentReplyReactions.count({ where: { reply_id, reaction: COMMON_REACTION_TYPES.LOVE } });
    const idea_count = await MarkerCommentReplyReactions.count({ where: { reply_id, reaction: COMMON_REACTION_TYPES.IDEA } });
    const confused_count = await MarkerCommentReplyReactions.count({ where: { reply_id, reaction: COMMON_REACTION_TYPES.CONFUSED } });
