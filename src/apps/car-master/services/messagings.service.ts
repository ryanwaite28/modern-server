...
import { paginateTable } from '../../_common/repos/_common.repo';
...
export class MessagingsService {
  ...
  static async get_user_messagings_all(user_id: number): ServiceMethodAsyncResults {
    const messagings_models = await CarmasterMessagings.findAll({
      where: {
        [Op.or]: [
          { user_id },
          { sender_id: user_id },
    ...
    for (const messaging of messagings_models) {
      ...
      const unread_messages_count = await CarmasterMessages.count({
        where: {
