...
export class PostCommentRepliesService {
  ...
  static async toggle_user_reaction(request: Request, response: Response) {
    ...
    if (!reply_reaction) {
      ...
      reply_reaction = await PostCommentReplyReactions.create({
        reaction,
        reply_id,
        owner_id: you.id
      });
  ...
  static async get_reply_reactions_counts(request: Request, response: Response) {
    ...
    const like_count = await PostCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.LIKE } });
    const love_count = await PostCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.LOVE } });
    const idea_count = await PostCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.IDEA } });
    const confused_count = await PostCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.CONFUSED } });
