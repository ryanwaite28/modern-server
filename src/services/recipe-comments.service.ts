...
import * as RecipeCommentsRepo from '../repos/recipe-comments.repo';
import {
  REACTION_TYPES,
  user_attrs_slim
} from '../chamber';
import { RecipeComments, RecipeCommentReactions } from '../models/recipe.model';
...
export class RecipeCommentsService {
  ...
  static async get_comment_reactions_counts(request: Request, response: Response) {
    ...
    const like_count = await RecipeCommentReactions.count({ where: { comment_id, reaction: REACTION_TYPES.LIKE } });
    const love_count = await RecipeCommentReactions.count({ where: { comment_id, reaction: REACTION_TYPES.LOVE } });
    const idea_count = await RecipeCommentReactions.count({ where: { comment_id, reaction: REACTION_TYPES.IDEA } });
    const confused_count = await RecipeCommentReactions.count({ where: { comment_id, reaction: REACTION_TYPES.CONFUSED } });
