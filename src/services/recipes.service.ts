...
import { Recipes, RecipePhotos, RecipeReactions, RecipeIngredients } from '../models/recipe.model';
import { Users } from '../models/user.model';
import { IRecipeModel } from 'src/model-types';
...
export class RecipesService {
  ...
  static async get_recipe_reactions_counts(request: Request, response: Response) {
    ...
    const like_count = await RecipeReactions.count({ where: { recipe_id, reaction: REACTION_TYPES.LIKE } });
    const love_count = await RecipeReactions.count({ where: { recipe_id, reaction: REACTION_TYPES.LOVE } });
    const idea_count = await RecipeReactions.count({ where: { recipe_id, reaction: REACTION_TYPES.IDEA } });
    const confused_count = await RecipeReactions.count({ where: { recipe_id, reaction: REACTION_TYPES.CONFUSED } });
    ...
    if (recipe_image) {
      ...
      if (!results.result) {
        ...
        if (recipe_model.get('image_id')) {
          try {
            await delete_cloudinary_image(recipe_model.get('image_id'));
          } catch (e) {
            console.log(e);
          }
        }
