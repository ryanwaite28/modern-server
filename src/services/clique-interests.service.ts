...
export class CliqueInterestsService {
  ...
  static async show_interest(request: Request, response: Response) {
    ...
    (<IRequest> request).io.emit(`${EVENT_TYPES.NEW_CLIQUE_INTEREST}:clique-${clique_id}`, {
      event_type: EVENT_TYPES.NEW_CLIQUE_INTEREST,
      data: { user: response.locals.you }
    });
    create_notification({
      from_id: you_id,
      to_id: response.locals.clique_model.get('creator_id'),
      event: EVENT_TYPES.NEW_CLIQUE_INTEREST,
  ...
  static async remove_interest(request: Request, response: Response) {
    ...
    (<IRequest> request).io.emit(`${EVENT_TYPES.CLIQUE_UNINTEREST}:clique-${clique_id}`, {
      event_type: EVENT_TYPES.CLIQUE_UNINTEREST,
      data: { user: response.locals.you }
    });
    return response.status(HttpStatusCode.OK).json({
