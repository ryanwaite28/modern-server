...
export class RecipeCommentRepliesService {
  ...
  static async get_reply_reactions_counts(request: Request, response: Response) {
    ...
    const like_count = await RecipeCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.LIKE } });
    const love_count = await RecipeCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.LOVE } });
    const idea_count = await RecipeCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.IDEA } });
    const confused_count = await RecipeCommentReplyReactions.count({ where: { reply_id, reaction: REACTION_TYPES.CONFUSED } });
