...
import { Recipes, RecipePhotos, RecipeIngredients } from '../models/recipe.model';
...
export async function create_recipe(createObj: {
  ...
  const new_recipe_model = await Recipes.create({
  ...
  const recipe_id = new_recipe_model.get('id');
